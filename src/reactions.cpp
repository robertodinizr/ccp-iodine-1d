...
std::shared_ptr<spark::collisions::Reactions<1, 3>> load_electron_reactions_iodine_1(
    const std::filesystem::path& dir,
    double neutral_temperature,
    spark::particle::ChargedSpecies<1, 3>* i_ions) {
    spark::collisions::Reactions<1, 3> ei_reactions;
    auto cfg = BasicCollisionConfig{.atomic_mass = spark::constants::m_i};

    // Reactions
    // 1. I_elastic
    ei_reactions.push_back(std::make_unique<ElectronElasticCollision<1, 3>>(
        cfg, load_cross_section(dir / "I_elastic.txt", 0.0)));

    // 2. I_excitation{1, 2, 3-25}
    ei_reactions.push_back(std::make_unique<ExcitationCollision<1, 3>>(
    ...
    // 3. I_ionization
    ei_reactions.push_back(std::make_unique<IonizationCollision<1, 3>>(
        i_ions, neutral_temperature, cfg, load_cross_section(dir / "I_ionization.txt", 12.13)));

    return std::make_shared<spark::collisions::Reactions<1, 3>>(std::move(ei_reactions));
}

class DissociativeAttachmentCollision final : public BasicCollision<1, 3> {
public:
    using BasicCollision<1, 3>::BasicCollision;

    DissociativeAttachmentCollision(spark::particle::ChargedSpecies<1, 3>* ions,
                                    const double t_neutral,
                                    const BasicCollisionConfig config,
                                    CrossSection&& cs)
        : BasicCollision<1, 3>(config, std::forward<CrossSection>(cs)),
          ions_(ions),
          t_neutral_(t_neutral) {};

    ReactionOutcome react(spark::particle::ChargedSpecies<1, 3>& projectile,
                          size_t id,
...
private:
    spark::particle::ChargedSpecies<1, 3>* ions_ = nullptr;
    double t_neutral_ = 0.0;
};

std::shared_ptr<spark::collisions::Reactions<1, 3>> load_electron_reactions_iodine_2(
    const std::filesystem::path& dir,
    double neutral_temperature,
    spark::particle::ChargedSpecies<1, 3>* i2_ions,
    spark::particle::ChargedSpecies<1, 3>* i_ions,
    spark::particle::ChargedSpecies<1, 3>* in_ions) {
    Reactions<1, 3> ei2_reactions;
    auto cfg = BasicCollisionConfig{.atomic_mass = 2 * spark::constants::m_i};

    // Reactions

    // 1. e+I2_elastic
    ei2_reactions.push_back(std::make_unique<ElectronElasticCollision<1, 3>>(
        cfg, load_cross_section(dir / "I2_elastic.txt", 0.0)));
    ei2_reactions.push_back(std::make_unique<ElectronElasticCollision<1, 3>>(
        cfg, load_cross_section(dir / "I2_elastic_momentum.txt", 0.0)));

    // 2. e+I2_excitation{1-4}
    ei2_reactions.push_back(std::make_unique<ExcitationCollision<1, 3>>(
    ...
    // 3. I2_ionization
    ei2_reactions.push_back(std::make_unique<IonizationCollision<1, 3>>(
    ...
    return std::make_shared<spark::collisions::Reactions<1, 3>>(std::move(ei2_reactions));
}

std::shared_ptr<spark::collisions::Reactions<1, 3>> load_atomic_ion_reactions_iodine_1(
    const std::filesystem::path& dir,
    double neutral_temperature,
    spark::particle::ChargedSpecies<1, 3>* i_ions) {
    auto r = std::make_unique<Reactions<1, 3>>();
    auto cfg = BasicCollisionConfig{.atomic_mass = spark::constants::m_i};

    r->push_back(std::make_unique<IonElasticCollision<1, 3>>(
        cfg, load_cross_section(dir / "I_I+_scattering.txt", 0.0)));

    r->push_back(std::make_unique<ChargeExchangeWithRemovalCollision<1, 3>>(
        i_ions, neutral_temperature, cfg,
...
std::shared_ptr<spark::collisions::Reactions<1, 3>> load_atomic_ion_reactions_iodine_2(
    const std::filesystem::path& dir,
    double neutral_temperature,
    spark::particle::ChargedSpecies<1, 3>* i2_ions) {
    auto r = std::make_unique<Reactions<1, 3>>();
    ...
    r->push_back(std::make_unique<IonDiffMassesElasticCollision<1, 3>>(
        spark::constants::m_i, 2 * spark::constants::m_i,
        load_cross_section(dir / "I2_I+_scattering.txt", 0.0)));

    r->push_back(std::make_unique<ChargeExchangeDiffCollision<1, 3>>(
...
std::shared_ptr<spark::collisions::Reactions<1, 3>> load_molecular_ion_reactions_iodine_1(
    const std::filesystem::path& dir,
    double neutral_temperature,
    spark::particle::ChargedSpecies<1, 3>* i2_ions) {
    auto r = std::make_unique<Reactions<1, 3>>();
...
std::shared_ptr<spark::collisions::Reactions<1, 3>> load_negative_ion_reactions_iodine_1(
    const std::filesystem::path& dir) {
    auto r = std::make_unique<Reactions<1, 3>>();
    auto cfg = BasicCollisionConfig{};

    r->push_back(std::make_unique<SinkCollision<1, 3>>(
        cfg, load_cross_section(dir / "I-_I+_volume_recombination.txt", 0.0)));

    return r;
}

std::shared_ptr<spark::collisions::Reactions<1, 3>> load_negative_ion_reactions_iodine_2(
    const std::filesystem::path& dir) {
    auto r = std::make_unique<Reactions<1, 3>>();
    auto cfg = BasicCollisionConfig{};
